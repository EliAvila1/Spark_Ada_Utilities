package body spark_handling_lowercase
with SPARK_Mode => On
is

   procedure lowercase (Item : in out String) is

      subtype to_sum     is Positive range 32 .. 32;
      subtype to_int_str is string(Item'First .. Item'Length);

      subtype conversion is Positive range 65 .. 90
        with Static_Predicate => conversion in 65 .. 90;

      subtype added is Positive range 97 .. 122
        with Static_Predicate => added in 97 .. 122;

      char_pos : conversion := 65;
      char_sum : added;

      added_sum    : constant to_sum     := 32;
      ...
      for I in Item'First .. Item'Last loop
         ...
         pragma Loop_Invariant (for all K in Item'First .. I - 1 =>
                                  (if Character'Pos(Item_Inicial(K)) in conversion
                                   then Character'Pos(Item(K)) = Character'Pos(Item_Inicial(K)) + added_sum
